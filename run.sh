...
elif [ $1 == "sim_lusplit_parallel" ]
then
	echo "Copy sim_lusplit_parallel to node3 & node4 ..."
	...
	echo "Run sim_lusplit_parallel ..."
	mpirun -np 16 -f mpi_config ./$1 2>> lusplit_parallel_out.txt
elif [ $1 == "sim_dishsum" ]
then
...
then
	...
	do
		...
		do
			...
			mpirun -np $i -f mpi_config ./$1 2>> prime_parallel_out.txt
			let n=$n*2
		done
	done
...
then
	...
	do
		...
		do
			...
			mpirun -np $i -f mpi_config ./$1 $n 2>> closure_parallel_out.txt
		done
	done
...
then
    ...
    for ((j = 1; j < 7; j ++));
		do
      let n=$n*2
			mpirun -np $i -f mpi_config ./$1 $n 2>> gauss_parallel_out.txt
		done
	done
...
then
    ...
    for ((j = 1; j < 16; j ++));
		do
      let n=$n*2
			mpirun -np $i -f mpi_config ./$1 $n 2>> fft_parallel_out.txt
		done
	done
elif [ $1 == "sim_alltoall" ]
then
